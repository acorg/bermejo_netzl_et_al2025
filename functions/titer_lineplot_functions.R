...
names(homologous_ags) <- tolower(names(homologous_ags))


calc_gmt_sr_effects_single_srg <- function(map, data_long, thresh, target_srg){
  
  data_long %>%
    filter(sr_group == target_srg) %>%
    pull(sr_name) -> target_sera
  
  test_table <- titerTable(subsetMap(map, sera = target_sera))
  
  test_res <- estimate_sr_effects(test_table, ci_level = 0.95, dilution_stepsize = 0)
  
  test_res <- as.data.frame(test_res[2:nrow(test_res),]) %>%
    mutate(variable = c(rownames(test_table), colnames(test_table)),
           logtiter = estimate) %>% 
    select(!estimate)
  
  # need to add titer and all below thresh to sr_group_gmt
  ag_means <- test_res[1:length(agNames(map)),] %>%
    mutate(titer = round(2^logtiter*10,2),
           ag_name = variable)
  ...
  data_long %>%
    ...
    left_join(ag_means, by = c("ag_name")) %>%
    mutate(sr_group = target_srg) -> ag_means
  
  
  sr_effects <- test_res[(length(agNames(map))+1):nrow(test_res),] %>%
    mutate(sr_group = target_srg,
           sr_effect = variable) %>%
    select(!variable)
  
  return(list("ag_means" = ag_means, "sr_effects" = sr_effects))
  
}

calc_gmt_sr_effects_all_srgs <- function(map, data_long, thresh){
  
  
  all_srgs <- unique(data_long$sr_group)
  
  ag_means_comb <- list()
  sr_effects_comb <- list()
  
  for(srg in all_srgs){
    
    temp_res <- calc_gmt_sr_effects_single_srg(map, data_long, thresh, srg)
    ag_means_comb[[srg]] <- temp_res$ag_means
    sr_effects_comb[[srg]] <- temp_res$sr_effects
  }
  
  ag_means_comb <- do.call(rbind, ag_means_comb)
  sr_effects_comb <- do.call(rbind, sr_effects_comb)
  
  return(list("ag_means" = ag_means_comb, "sr_effects" = sr_effects_comb))
}


calc_fc_from_homologous <- function(data_long, titer_adjusted = FALSE){
  ...
  for(sr in unique(data_long$sr_group)){
    ...
    if(!ag1 %in% tolower(ag_names)){
      ag1 <- tolower(homologous_ags)[ag1]
    }
    
    if(ag1 %in% tolower(ag_names) | ag1 %in% tolower(homologous_ags)){
  ...
  for(sr in unique(data_long$sr_group)){
    ...
    if(ag1 %in% tolower(ag_names) | ag1 %in% tolower(homologous_ags)){
      ...
      for(ag2 in ag_names){
        ...
        if(length(titers1)==0 | length(titers2) == 0){
          next()
        }
...
calc_titertools_sr_group_gmt <- function(data_long, thresh, grouping_vars = c('sr_group', 'ag_name')){
...
calc_half_thresh_sr_group_gmt <- function(data_long, thresh, grouping_vars = c("sr_group", "ag_name")){
...
do_titer_plot_fc_label <- function(map, facet_col = 3, thresh = 20, fc_label =TRUE, adj_titers = TRUE,
                                   ...
                                   order_by_gmt = FALSE,
                                   use_ag_number = FALSE,
                                   gmt_grouping_vars = c('sr_group', 'ag_name')) {
  ...
  if(is.null(sr_group_gmt_plotdata)){
    if(titertools_gmt){
      sr_group_gmt_plotdata <- calc_titertools_sr_group_gmt(plotdata, thresh = thresh, grouping_vars = gmt_grouping_vars)
    } else {
      sr_group_gmt_plotdata <- calc_half_thresh_sr_group_gmt(plotdata, thresh = thresh, grouping_vars = gmt_grouping_vars)
    }
  ...
  if(!facet_var %in% colnames(sr_group_gmt_plotdata) & show_gmt){
    ...
    plotdata_facet <- unique(as.character(plotdata[[facet_var]]))
    
    if(length(plotdata_facet) < 2) {
      sr_group_gmt_plotdata[[facet_var]] <- plotdata_facet
    } else {
  ...
  if(show_gmt_conf){
    ...
    sr_group_gmt_plotdata %>%
      mutate(lower = ifelse(is.na(lower), logtiter, lower),
             upper = ifelse(is.na(upper), logtiter, upper)) -> sr_group_gmt_plotdata
    gp + 
      geom_pointrange(data = sr_group_gmt_plotdata,
                      aes_string(ymin = "lower",
                          ...
                          group = color_var),
                      alpha = 1,
                      position = position_dodge(width = 0.3))  -> gp
  } 
  ...
  gp + scale_color_manual(
    ...
    labs(
      x = ifelse(use_ag_number, "Rank", "Antigen variant")
    ) +
    coord_cartesian(
...
do_fold_change_boxplot <- function(data_fc, sr_group_colors, x_var = "sr_group", color_var = "sr_group", yrange = 5){
  ...
  data_fc %>%
    ggplot(aes_string(x = x_var, y = "fc", color = color_var)) +
    geom_boxplot() +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_jitter(alpha = 0.6) +
    scale_color_manual(values = color_vals) +
    scale_y_continuous(name = "Fold change",
                       labels = function(x) ifelse(x < 0, paste0("-", round(2^x, 2)), round(2^x, 2)),
                       breaks = c(-yrange:yrange),
                       limits = c(-yrange, yrange)) +
    theme_bw() +
...
do_titerplot_gmt <- function(comb_gmt_adj, map, sub_ags, thresh, color_vals){
  
  ymin <- min(c(-0.5, log2(thresh/20)-0.5))
  
  comb_gmt_adj %>%
    ggplot(aes(x = ag_name, y = logtiter, group = sr_name, color = sr_name)) + 
    geom_pointrange(aes(ymin = lower, ymax = upper), position = position_dodge(width = 0.2)) +
    geom_line(position = position_dodge(width = 0.2)) + 
    geom_point(color = "white", position = position_dodge(width = 0.2), size = 0.8) +
    ...
    theme_bw() + 
    facet_wrap(~sr_group) +
    coord_cartesian(ylim = c(ymin, 10)) +
    theme(strip.background.x = element_blank(),
          axis.text.x = element_text(angle = 45, hjust = 1)) -> p_gmt
...
# gmt barplots
do_titer_barplot <- function(data, gmt_sub, ymax = 7, threshold = 16){
  
  data %>%
  ggplot(aes(x = sr_group, y = logtiter, group = sr_name, color = sr_group, fill = sr_group)) + 
    geom_bar(data = gmt_sub, stat = "identity", color = "white") +
    geom_point(position = position_dodge(width = 0.7), shape = 21, color = "white") + 
    geom_rect(aes(ymin = -Inf, ymax = log2(1.6), xmin = -Inf, xmax = Inf), alpha = 0.008, color = NA, fill = "grey60") +
    scale_color_manual(values = plot_colors) +
    scale_fill_manual(values = plot_colors) +
    scale_y_continuous(
      limits = c(-1, ymax),
      breaks = log2(threshold/10):(log2(threshold/10)+ymax),
      labels = function(x) {
        output <- 2^x*10
        output[x == log2(threshold/10)] <- paste0("<", threshold)
        output
      },
      name = "Titer"
    ) +
