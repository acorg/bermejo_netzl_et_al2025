...
# Titer table modification functions
optimize_map_new_titer_table <- function(map, titer_table, options_list = list(ignore_disconnected = TRUE)){
  
  map_modified <- map
  titerTable(map_modified) <- titer_table
  map_modified <- optimizeMap(map_modified, 2, 1000, options = options_list)
  map_modified <- realignMap(map_modified, map)
  
  return(map_modified)
  
}

set_titrations_titertable <- function(map_table, vax_mod_table){
  for(sr in colnames(vax_mod_table)){
    
    map_table[rownames(vax_mod_table), sr] <- vax_mod_table[,sr]
  }
  
  map_table[is.na(map_table)] <- "*"
  
  return(map_table)
  
}

set_titrations_value <- function(table, value, variants, sera){
  
  old_table <- table
  table[variants, sera] <- value
  
  old_table[which(old_table != "*", arr.ind = TRUE)] <- table[which(old_table != "*", arr.ind = TRUE)]
